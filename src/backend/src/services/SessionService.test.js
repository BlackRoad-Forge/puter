...
describe('SessionService', async () => {
    ...
    const clearSessionState = async (sessionUuid) => {
        if ( sessionUuid ) {
            await redisClient.del(`session-cache:${sessionUuid}`);
            delete sessionService.sessions[sessionUuid];
    ...
    it('caches sessions in redis on create with five-minute ttl', async () => {
        ...
        try {
            ...
            await clearSessionState(session.uuid);
        }
    });
    ...
    it('loads session user uid using object lookup options', async () => {
        ...
        delete sessionService.sessions[session.uuid];
